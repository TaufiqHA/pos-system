...
@section('title', 'Daftar Satuan - Lucifer POS')

@section('page_title', 'Daftar Satuan')
@section('page_subtitle', 'Mengelola satuan produk POS')
        ...
        <table class="w-full text-left border-collapse whitespace-nowrap">
            <thead>
                <tr class="border-b border-gray-800 text-gray-400 text-xs font-bold uppercase tracking-wider">
                    ...
                    <th class="pb-3 px-4">Nama Satuan</th>
            ...
            <tbody class="divide-y divide-gray-800 text-xs text-gray-300">
                @forelse($units as $unit)
                    <tr class="hover:bg-gray-800/30 transition">
                        ...
                        <td class="py-4 px-4 font-semibold text-white">{{ $unit->name }}</td>
                        <td class="py-4 pl-4 pr-4 text-right whitespace-nowrap">
                            <div class="flex justify-end items-center gap-2">
                                <button onclick="openDetailModal({{ json_encode($unit) }})" class="text-blue-400 hover:text-blue-300 font-semibold transition px-2 py-1 hover:bg-blue-500/10 rounded cursor-pointer">
                                    Detail
                                </button>
                                <button onclick="openEditModal({{ json_encode($unit) }})" class="text-yellow-400 hover:text-yellow-300 font-semibold transition px-2 py-1 hover:bg-yellow-500/10 rounded cursor-pointer">
                                    Edit
                                </button>
                                <form action="{{ route('units.destroy', $unit->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus satuan ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="3" class="py-8 text-center text-gray-500">Belum ada satuan terdaftar.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="create-modal" class="fixed inset-0 z-50 {{ $errors->any() && !old('_method') ? '' : 'hidden' }} bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="card max-w-md w-full p-6 rounded-2xl shadow-2xl relative border border-gray-800">
        ...
        <div class="mb-6">
            <h3 class="text-base font-bold tracking-wide font-display text-white">Tambah Satuan Baru</h3>
            <p class="text-[11px] text-gray-400 mt-1">Buat satuan baru untuk produk</p>
        </div>
        <form action="{{ route('units.store') }}" method="POST" class="space-y-4 text-xs">
            @csrf
            <div class="space-y-1">
                <label for="create-name" class="block font-bold text-gray-300">Nama Satuan</label>
                <input type="text" name="name" id="create-name" value="{{ !old('_method') ? old('name') : '' }}" placeholder="Contoh: Pcs, Box, Kg, Liter" class="w-full bg-gray-900 border border-gray-800 text-white rounded-xl p-3 focus:outline-none focus:border-green-400 @if($errors->any() && !old('_method')) border-red-500 @endif">
...
<div id="edit-modal" class="fixed inset-0 z-50 {{ $errors->any() && old('_method') === 'PUT' ? '' : 'hidden' }} bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="card max-w-md w-full p-6 rounded-2xl shadow-2xl relative border border-gray-800">
        ...
        <div class="mb-6">
            <h3 class="text-base font-bold tracking-wide font-display text-white">Edit Satuan</h3>
            <p class="text-[11px] text-gray-400 mt-1">Ubah nama satuan yang sudah ada</p>
        </div>
        <form id="edit-form" action="{{ old('id') ? route('units.update', old('id')) : '#' }}" method="POST" class="space-y-4 text-xs">
            @csrf
            @method('PUT')
            <input type="hidden" name="id" id="edit-id" value="{{ old('id') }}">

            <div class="space-y-1">
                <label for="edit-name" class="block font-bold text-gray-300">Nama Satuan</label>
                <input type="text" name="name" id="edit-name" value="{{ old('_method') === 'PUT' ? old('name') : '' }}" placeholder="Contoh: Pcs, Box, Kg, Liter" class="w-full bg-gray-900 border border-gray-800 text-white rounded-xl p-3 focus:outline-none focus:border-green-400 @if($errors->any() && old('_method') === 'PUT') border-red-500 @endif">
...
<div id="detail-modal" class="fixed inset-0 z-50 hidden bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="card max-w-md w-full p-6 rounded-2xl shadow-2xl relative border border-gray-800">
        ...
        <div class="mb-6">
            <h3 class="text-base font-bold tracking-wide font-display text-white">Detail Satuan</h3>
            <p class="text-[11px] text-gray-400 mt-1">Informasi lengkap satuan</p>
        </div>
        <div class="space-y-4 text-xs">
            <div class="bg-gray-900/50 p-4 rounded-xl space-y-3">
                <div>
                    <p class="text-gray-500 font-bold uppercase tracking-wider text-[9px]">ID Satuan</p>
                    <p id="detail-id" class="text-white font-mono mt-0.5 text-sm"></p>
                </div>
                <div>
                    <p class="text-gray-500 font-bold uppercase tracking-wider text-[9px]">Nama Satuan</p>
...
<script>
    ...
    function openEditModal(unit) {
        document.getElementById('edit-id').value = unit.id;
        document.getElementById('edit-name').value = unit.name;
        document.getElementById('edit-form').action = `/admin/units/${unit.id}`;
    ...
    function openDetailModal(unit) {
        document.getElementById('detail-id').textContent = unit.id;
        document.getElementById('detail-name').textContent = unit.name;
        
        const createdDate = unit.created_at ? new Date(unit.created_at).toLocaleString('id-ID') : '-';
        const updatedDate = unit.updated_at ? new Date(unit.updated_at).toLocaleString('id-ID') : '-';
