...
@section('content')
    ...
    <div class="block md:hidden space-y-6 mb-8 z-10">
        ...
        <div class="bg-[#111827] border border-gray-800 rounded-3xl p-6 shadow-xl text-center">
            ...
            <div class="mt-6 space-y-4 text-left">
                <div class="flex justify-between items-center pb-3.5 border-b border-gray-800">
                    ...
                    <span class="text-sm font-extrabold text-white font-display">Rp {{ number_format($omsetPenjualan, 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between items-center pt-0.5">
                    <span class="text-xs font-bold text-gray-300 tracking-wider uppercase">HUTANG KE PUSAT</span>
                    <span class="text-sm font-extrabold text-yellow-500 font-display">Rp {{ number_format($hutangPusat, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6 my-8">
            <!-- Box 1: Produk -->
            <a href="{{ route('cabang.monitoring-stok') }}" class="flex flex-col items-center group">
                <div class="w-full aspect-square flex items-center justify-center bg-white rounded-[2rem] shadow-lg group-hover:scale-105 transition-transform p-6 mb-3">
                    <img src="https://cdn-icons-png.flaticon.com/512/3144/3144456.png" class="w-14 h-14 object-contain" alt="Produk">
                </div>
            ...
            <!-- Box 2: Penjualan -->
            <a href="{{ route('cabang.penjualan') }}" class="flex flex-col items-center group">
                <div class="w-full aspect-square flex items-center justify-center bg-white rounded-[2rem] shadow-lg group-hover:scale-105 transition-transform p-6 mb-3">
                    <img src="https://cdn-icons-png.flaticon.com/512/3135/3135706.png" class="w-14 h-14 object-contain" alt="Penjualan">
                </div>
                <span class="text-[11px] font-bold text-white tracking-widest text-center uppercase group-hover:text-[#B4F481] transition-colors leading-tight">
                    PENJUALAN
                </span>
            ...
            <a href="{{ route('purchase-orders.index') }}" class="flex flex-col items-center group">
                <div class="w-full aspect-square flex items-center justify-center bg-white rounded-[2rem] shadow-lg group-hover:scale-105 transition-transform p-6 mb-3">
                    <img src="https://cdn-icons-png.flaticon.com/512/869/869636.png" class="w-14 h-14 object-contain" alt="Daftar PO">
                </div>
                <span class="text-[11px] font-bold text-white tracking-widest text-center uppercase group-hover:text-[#B4F481] transition-colors leading-tight">
                    DAFTAR PO
                </span>
            ...
            <!-- Box 4: Hutang -->
            <a href="{{ route('cabang.hutang') }}" class="flex flex-col items-center group">
                <div class="w-full aspect-square flex items-center justify-center bg-white rounded-[2rem] shadow-lg group-hover:scale-105 transition-transform p-6 mb-3">
                    <img src="https://cdn-icons-png.flaticon.com/512/2916/2916315.png" class="w-14 h-14 object-contain" alt="Hutang">
                </div>
                <span class="text-[11px] font-bold text-white tracking-widest text-center uppercase group-hover:text-[#B4F481] transition-colors leading-tight">
                    HUTANG
                </span>
        ...
        <div class="space-y-4">
            <button onclick="openPermintaanPoModal()"
                class="w-full flex items-center justify-center px-6 py-4 rounded-2xl border-2 border-yellow-400 text-yellow-400 font-bold text-xs tracking-widest hover:bg-yellow-400 hover:text-black transition cursor-pointer relative uppercase bg-transparent">
    ...
    <script>
        ...
        function handleBannerClick() {
            if (upcomingProductsData.length > 0 && upcomingProductsData[currentSlideIndex]) {
                openDetailUpcomingProductModalCabang(upcomingProductsData[currentSlideIndex].id);
            }
        }
        function initUpcomingProductsBanner() {
            fetch('/cabang/upcoming-products', {
                headers: {
                    'Accept': 'application/json',
        ...
        function openDetailUpcomingProductModalCabang(id) {
            fetch(`/cabang/upcoming-products`, {
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
            ...
            .then(data => {
                ...
                const img = document.getElementById('detail-upcoming-image-cabang');
                if (item.image) {
                    img.src = item.image;
                ...
                document.getElementById('detail-upcoming-name-view-cabang').innerText = item.name;
                document.getElementById('detail-upcoming-description-view-cabang').innerText = item.description || '-';

                document.getElementById('detail-upcoming-product-modal-cabang').classList.remove('hidden');
            })
            .catch(error => {
        ...
        function closeDetailUpcomingProductModalCabang() {
            document.getElementById('detail-upcoming-product-modal-cabang').classList.add('hidden');
        }
    ...
    <div id="detail-upcoming-product-modal-cabang"
        ...
        <div class="card max-w-md w-full p-6 rounded-2xl shadow-2xl relative border border-gray-850">
            <button onclick="closeDetailUpcomingProductModalCabang()"
                class="absolute top-4 right-4 text-gray-400 hover:text-white transition cursor-pointer">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
            ...
            <div class="space-y-4 text-xs text-gray-300">
                ...
                <div class="flex justify-center bg-gray-950 p-4 rounded-xl border border-gray-800">
                    <img id="detail-upcoming-image-cabang" src="" alt="Upcoming Product Image" class="max-h-48 rounded-lg object-contain">
                </div>

                <div class="space-y-1">
                    <span class="text-gray-400 text-[10px] uppercase font-bold tracking-wider font-display">Nama Produk</span>
                    <p id="detail-upcoming-name-view-cabang" class="text-sm font-bold text-white"></p>
                </div>

                <div class="space-y-1">
                    <span class="text-gray-400 text-[10px] uppercase font-bold tracking-wider font-display">Deskripsi</span>
                    <p id="detail-upcoming-description-view-cabang" class="leading-relaxed bg-gray-900/60 p-3 rounded-xl border border-gray-800/50 whitespace-pre-line"></p>
                </div>
            </div>

            <div class="flex justify-end pt-6">
                <button type="button" onclick="closeDetailUpcomingProductModalCabang()"
                    class="bg-gray-800 hover:bg-gray-700 text-white font-bold py-2.5 px-6 rounded-xl transition cursor-pointer font-sans">
                    Tutup
                </button>
            </div>
        </div>
