...
<!-- Header Section -->
<div class="mb-6">
    <h1 class="text-2xl font-extrabold tracking-tight text-white font-display">Hutang Cabang</h1>
    <p class="text-xs text-gray-400 mt-1">Daftar hutang cabang kepada Pusat (PO Kredit) dan riwayat pembayarannya.</p>
</div>
...
<div class="card p-6 rounded-2xl shadow-xl border border-gray-800 bg-[#1F2937]/35 backdrop-blur-sm">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
            <!-- Filter Status -->
            <select id="filter-status" onchange="filterDebts()" class="bg-gray-900 border border-gray-800 text-white rounded-xl p-2.5 text-xs focus:outline-none focus:border-[#B4F481]">
        ...
        <table class="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody id="debts-table-body" class="divide-y divide-gray-800 text-xs text-gray-300">
                @forelse($debts as $debt)
                    <tr class="hover:bg-gray-800/30 transition debt-row" data-status="{{ $debt->status }}">
                        ...
                        <td class="py-4 pl-4 pr-4 text-right whitespace-nowrap">
                            <div class="flex justify-end items-center gap-2">
                                ...
                                @if(!$hasPending && $debt->status !== 'paid')
                                    <button onclick="openPaymentModal({{ json_encode($debt) }})" class="text-[#B4F481] hover:text-green-300 font-semibold transition px-2.5 py-1 hover:bg-green-500/10 rounded border border-[#B4F481]/20 cursor-pointer">
                                        Bayar / Transfer
                                    </button>
                                @endif
                            </div>
